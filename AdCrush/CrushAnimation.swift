...
struct CrushAnimation {
  ...
  init(velocity: Float, direction: CrushDirection) {
    self.velocity = velocity
    self.direction = direction
    // This could use some improving
    self.length =  {
      let speed = 100 / velocity
      guard speed <= 0.4 else { return 0.4 }
      guard speed >= 0.1 else { return 0.1 }
      return speed }()
//    print("absolute velocity", velocity)
     calculateAction(direction: direction)
  }
  
  mutating func calculateAction(direction: CrushDirection) {
    setupSourceGrid()
  ...
  private mutating func determineVectorShift() -> (float2) -> float2 {
  ...
  private mutating func setupSourceGrid() {
    source = [
